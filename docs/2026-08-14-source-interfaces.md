...
**X 大V扫描（2026-08-18 用户要求新增，含当日实测；可信度见 `docs/x-influencer-credibility.md`）：** 每日 X 采集除用户 Home timeline 外，追加读取以下大V主页最新帖。**采集通道用 WebFetch 直读 `x.com/<handle>`（2026-08-18 实测有效，未登录可读公开帖；不依赖 IAB webview，规避其故障状态）**。**引用规则：报告正文必须标注具体 `@handle（称呼，可信度标签）`，禁止写"X平台"式笼统来源；单源（仅一个大V）只能进待核验线索，官方媒体号才允许直接引用入主榜。** 清单以**财经/市场/科技**方向为主，均为公开知名账号：
...
| `WSJmarkets` | WSJ 市场官方 | 市场快讯/宏观 | ✅ 5 条今日帖（全球债券收益率多年新高、油价>90债券抛售深化、高盛4.1亿收购） |
| `business` | 彭博官方 | 全球商业/市场（契合宇树/港股） | ✅ 5 条今日帖（宇树机器狗、LimX港股IPO、美国7月工业产出+0.2%） |
| `ZeroHedge` | 新闻聚合高流量号 | 突发/反共识观点 | ✅ 5 条今日帖（家得宝财报、霍尔木兹油价回撤、高盛警告美联储或被迫加息） |
| `SquawkCNBC` | CNBC 开市晨间官方 | 隔夜/盘前公司焦点 | ✅ 5 条今日帖（英伟达俄亥俄1000亿融资、比特币） |
| `markets` | 彭博市场官方 | 美股/全球市场快讯 | ⚠️ 只读到 2025 旧帖（官方号低频） |
| `RampCapitalLLC` | Sahm 规则作者 | 宏观（偏闲聊） | ⚠️ 可用但内容多为生活/观点帖，财经数据流少 |
| `LizAnnSonders` | Schwab 首席投资策略师 | 美股策略与情绪 | 未实测 |
| `CathieDWood` | ARK Invest CEO | AI/创新科技主线 | 未实测 |
| ~~`charliebitar`~~ | ~~Charlie Bilello~~ | ~~宏观数据~~ | ❌ handle 无效（0 posts，弃用，勿编造） |
| ~~`vtcheung`~~ | ~~科技分析师~~ | ~~美股科技~~ | ❌ 404 不存在，弃用 |
| ~~`ClaudiaSahm`~~ | ~~Sahm 法则作者~~ | ~~宏观~~ | ❌ 0 posts（真实活跃号另有 handle，勿猜） |
