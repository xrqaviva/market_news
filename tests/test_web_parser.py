...
class WebReportParserTest(unittest.TestCase):
    ...
    def test_all_report_model_field_counts_do_not_regress_during_legacy_recovery(self):
        ...
        self.assertEqual(347, len(items))
        self.assertEqual(593, sum(len(item.sources) for item in items))
        self.assertEqual(347, sum(bool(item.core) for item in items))
        self.assertEqual(287, sum(bool(item.score_breakdown) for item in items))
        self.assertEqual(252, sum(bool(item.signal) for item in items))
        self.assertEqual(252, sum(bool(item.market_feedback) for item in items))
        self.assertEqual(252, sum(bool(item.boundary) for item in items))
        self.assertEqual(50, sum(bool(item.variables) for item in items))
        self.assertEqual(267, sum(bool(item.heat_change) for item in items))
        self.assertEqual(202, sum(bool(item.release_session) for item in items))
